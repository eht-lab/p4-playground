...
class ExerciseTopo(Topo):
    ...
    """
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, bridges, **opts):
        Topo.__init__(self, **opts)
        host_links = []
        switch_links = []

        # assumes host always comes first for host<-->nic links
        for link in links:
            if link['node1'][0] == 'h':
        ...
        for sw, params in bridges.items():
            self.addSwitch(sw, log_file="%s/%s.log" %(log_dir, sw), cls=LinuxBridge)
...
class ExerciseRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
        ...
        self.switches = topo['nics']
        if "bridges" in topo:
            self.bridges = topo['bridges']
        else:
            self.bridges = {}
    ...
    def parse_links(self, unparsed_links):
        ...
        for link in unparsed_links:
            ...
            if link_dict['node1'][0] == 'h':
                assert link_dict['node2'][0] == 'n', 'Hosts should be connected to switches, not ' + str(link_dict['node2'])
    ...
    def create_network(self):
        ...
        self.topo = ExerciseTopo(self.hosts, self.switches, self.links, self.log_dir, self.bmv2_exe, self.pcap_dir, self.bridges)
    ...
    def do_net_cli(self):
        ...
        for s in self.net.switches:
            if not isinstance(s, LinuxBridge):
                s.describe()
